...
/// Callback for decoded frames (runs on MainActor)
typealias DecodedFrameCallback = @MainActor (CVPixelBuffer, CMTime) -> Void
...
class LowLatencyDecoder {
    ...
    deinit {
        // Clean up session synchronously - safe because VT calls are thread-safe
        // Access MainActor-isolated property using assumeIsolated pattern
    ...
    private func tryConfigureSession() {
        ...
        var outputCallback = VTDecompressionOutputCallbackRecord(
            decompressionOutputCallback: { (decompressionOutputRefCon, _, status, _, imageBuffer, pts, _) in
                ...
                decoder.handleDecodedFrameSync(status: status, imageBuffer: imageBuffer, presentationTimeStamp: pts)
            },
    ...
    /// Handle decoded output from VideoToolbox (called from VT callback thread)
    nonisolated func handleDecodedFrameSync(status: OSStatus, imageBuffer: CVImageBuffer?,
        ...
        // CVPixelBuffer is thread-safe for reading, wrap to satisfy Sendable
        nonisolated(unsafe) let safeBuffer = pixelBuffer

        // Dispatch to main queue for callback
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            MainActor.assumeIsolated {
                self.decodedCallback?(safeBuffer, pts)
            }
        }
    }
}
